...
import { SITE_TYPE } from './github-constants'
...
async function getGitHubRepoInfo(repoPath: string): Promise<GitHubRepoInfo> {
  return withFallback([
    async () => {
...
export async function getSiteStatus(
  ...
  try {
    ...
    switch (type) {
      case 'github': {
        const { repo, latestRelease, packages: repoPackages } = await getGitHubRepoInfo(path)

        let latestPackage = await resolvePackage(path, pkgname, repoPackages)

        return {
          name,
          url,
          desc,
          type: SITE_TYPE.GITHUB,
...
export async function getAllSitesStatus(
  sites: { name: string; url: string; desc?: string; pkgname?: string }[]
): Promise<SiteStatus[]> {
  return Promise.all(
    sites.map(site => getSiteStatus(site.name, site.url, site.desc, site.pkgname))
  )
}
